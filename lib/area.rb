...
module Area
  ...
  SUB_AREA_ZIP = [["200","201","202","203","204","205","206"],
  ...
  def self.show_area(obj , is_brake = nil)
    return area_name(obj.main_area , obj.sub_area , is_brake)
  end
  def self.show_zip(obj)
    if obj.main_area && obj.sub_area
      return SUB_AREA_ZIP[obj.main_area][obj.sub_area]
    else
      return "未設定"
    end
  end

  def self.area_name(main_index , sub_index , is_brake = false)
    if main_index.is_a?(Fixnum)
      temp_name = MAIN_AREA[main_index]
      temp_name += "#{"<br/>" if is_brake}#{SUB_AREA[main_index][sub_index]}" if sub_index.is_a?(Fixnum)
      return temp_name
    else
      return "未設定"
    end
